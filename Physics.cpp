...
void SoftBody::update(float deltaTime) {
    ...
    for (int i = 0; i < subSteps; i++) {
        ...
        updateEngine(subStepTime);
        updateTransmission(subStepTime);
        updateDifferential(subStepTime);
        ...
        for (auto& node : nodes) {
            node.velocity[0] += node.force[0] / node.mass * subStepTime;
            node.velocity[1] += node.force[1] / node.mass * subStepTime;
            node.velocity[2] += node.force[2] / node.mass * subStepTime;

            node.position[0] += node.velocity[0] * subStepTime;
            node.position[1] += node.velocity[1] * subStepTime;
            node.position[2] += node.velocity[2] * subStepTime;
    ...
    if (steeringAngle != 0.0f) {
        for (auto& wheel : wheels) {
            if (wheel.isDriven) {
                ...
                Vector3 newDirection = Vector3RotateByAxisAngle(
                    direction,
